...
Then /^(?:|I )should( not)? see ("[^"]*"|'[^']*')(?: within "([^\"]*)")?$/ do |negate, text, selector|
  with_scope(selector) do
    text = text[1..-2]
    escaped = Regexp.escape(text).gsub('\ ', '(?:\0|\\\302\\\240|[ \n]+)')
    
    regex = if escaped.respond_to?(:force_encoding)
      Regexp.new(escaped.force_encoding("UTF-8"), Regexp::FIXEDENCODING)
    else
      Regexp.new(escaped)
    end
    
    args = ['/.', { :text => regex, :visible => Capybara.current_driver == Capybara.javascript_driver }]
